function [fval, k, L, t_0, A, B] = parameters_optimisation_stage4(data)
 
% Optimisation Stage 4: 
%   Set Beta as a logistic function (Beta = L / (1 + exp( (k) * (t - t_0)))),
%   and Gamma as an exponential (Gamma = A * exp(B * t)).
%   Solve for L, k, t_0, A, and B to minimise the residual.
%   Here, the output fval is the residual we obtained. 
    ...
    % Initial guess for parameters [k, L, t_0, A, B]
    initial_guess = ...
        [0.5 + rand() * 0.01; 
        2 + rand() * 0.01; 
        10 + rand() * 0.01; 
    ...
    % Initial conditions
    init_cond = [S(1) I(1)];
    [~,y] = ode15s(@(t,y) ode_sys(t, y, k, L, t_0, A, B, N), t, init_cond);
    ...
    % cumulative MASH attendance (I) and the solution to the SI model.  
        ...
        [~,y_1] = ode15s(@(t,y) ode_sys(t, y, k_1, L_1, t_0_1, A_1, B_1, N), t_int_1, init_cond_1); 
...
function dydt = ode_sys(t, y, k, L, t_0, A, B, NN)

    % dS / dt = - beta * S * I / N;
    % dI / dt = beta * S * I / N - gamma * I;
    % Here, we set beta = L / (1 + exp( (k) * (t - t_0))); and 
    % Gamma = A * exp(B * t).
    %
    % Inputs: 
    % y = [S I];
    ...
    dydt = [ - L / (1 + exp( (k) * (t - t_0))) * y(1) * y(2) / NN;
        L / (1 + exp( (k) * (t - t_0))) * y(1) * y(2) / NN - A * exp(B * t) * y(2) ];
end
